...
router.get(
	...
	async function (request: Request, response: Response, next: NextFunction) {
		if (!request.body.user_id) {
			...
			const document: any = await usersystem_controller.searchUserById(
				request.body.user_id,
			);
			if (document._id) {
...
router.get(
	...
	async function (request: Request, response: Response, next: NextFunction) {
		if (!request.body.email) {
			...
			const document: any = await usersystem_controller.searchUserByEmail(
				request.body.email,
			);
			if (document._id) {
				responses.Success(request, response, document, 200);
...
router.post(
	...
	async function (request: Request, response: Response, next: NextFunction) {
		if (
			...
			const transaction: any = await usersystem_controller.addUserSystem(
				request.body,
			);
	...
	async function (request: Request, response: Response, next: NextFunction) {
		if (!request.body.user_id || !request.body.new_little_description) {
			...
			const transaction: any =
				await usersystem_controller.updateLittleDescriptionOfUser(
					request.body.user_id,
			...
			if (transaction === 1) {
				responses.Success(
					request,
					response,
					"Descripcion del usuario actualizada...!",
					200,
				);
			}
			if (transaction === 2) {
...
router.delete(
	...
	async function (request: Request, response: Response, next: NextFunction) {
		if (!request.body.user_id) {
			...
			const transaction: any = await usersystem_controller.deleteUserSystem(
				request.body.user_id,
			);
			if (transaction === 1) {
